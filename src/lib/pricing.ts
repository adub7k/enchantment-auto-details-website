...
/**
 * The tenant uses the `detail` industry profile, so categories are the
 * platform's (`detail`, `wash`…) or whatever the owner types. Two ways a
 * service lands on a page: a recognised category, or — failing that — its
 * NAME ("Bronze", "Interior Detail Only", "Exterior Foam Wash"). Anything
 * that matches neither is simply not shown. LAUNCH.md tells the owner how to
 * name things.
 */
...
const NAME_MATCHERS: [RegExp, string][] = [
  [/bronze|silver|gold|full detail|full-detail|inside.?(and|&).?out|package/i, "full-detail"],
  [/interior|vacuum|shampoo|headliner|seat|carpet/i, "interior-detail"],
  [/wash|foam|exterior|spot.?free/i, "exterior-wash"],
];
...
export function fetchPricing(): Promise<Pricing | null> {
  // Owner's call: no prices on the site. Every consumer treats null as
  // "render nothing", so flipping site.pricing.published is the only switch.
  if (!site.pricing.published) return Promise.resolve(null);
  if (!pricingPromise) {
